...
counts_per_hour = {}  # Dictionary to store hourly counts with shift labels
button_previous = GPIO.HIGH  # To detect button press
start_time = datetime.now()
time_data = []
count_data = []

# Shift timing classification
def get_shift():
    now = datetime.now()
    hour = now.hour
...
def update_graph(frame):
    ...
    ax2.bar(hours, values, color='blue')
...
def read_button():
    ...
    while True:
        ...
        if button_state == GPIO.LOW and button_previous == GPIO.HIGH:
            ...
            shift_label = get_shift()  # Get shift label
            hour_key = f"{shift_label} - {current_hour}:00"
            
            # Store count per hour with shift info
            if hour_key not in counts_per_hour:
                counts_per_hour[hour_key] = 0
            counts_per_hour[hour_key] += 1
