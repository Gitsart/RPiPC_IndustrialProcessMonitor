...
start_time_label = Label(root, text="Start Time: {start_time.strftime('%H:%M:%S')}", font=("Arial", 16))
...
def capture_and_save_images():
    ...
    filename = f"image_timestamp.jpg"
    file_path = os.path.join(folder_path, filename)
    
    #CaptureImage
    try:
        subprocess.run(["rpicam-still", "-o", file_path, "--width 720", "--height 720"], check=True)
...
def read_button():
    ...
    while True:
        ...
        #    Kaynes TB AUTOMATION MACHINE MONITOR by SSM
