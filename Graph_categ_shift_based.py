...
def update_graph(frame):
    ax.clear()
    ax.set_xlabel("Date and Shift")
    ax.set_ylabel("Production Count")
    ax.set_title("Production Count Per Shift")
    ...
    bars = ax.bar(shift_labels, values, color=colors)
    
    # Add count values inside bars
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height()/2, str(value), ha='center', va='center', color='white', fontsize=12, fontweight='bold')
